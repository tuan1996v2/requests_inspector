...
class RequestsInspectorInterceptor extends Interceptor {
  ...
  Future<void> onResponse(
    ...
    InspectorController().addNewRequest(
      RequestDetails(
        requestMethod: _parseMethod(response.requestOptions.method),
        url: url,
        statusCode: response.statusCode ?? 0,
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    InspectorController().addNewRequest(
      RequestDetails(
        requestMethod: _parseMethod(err.requestOptions.method),
        url: url,
  ...
  RequestMethod _parseMethod(String? method) {
    if (method == null || method.isEmpty) return RequestMethod.GET;
  ...
  RequestDetails _convertToRequestDetails(RequestOptions options) =>
      RequestDetails(
        requestMethod: _parseMethod(options.method),
